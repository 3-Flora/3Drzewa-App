...
const Navigation = () => {
  ...
  return (
    <>
      {/* Desktop Navigation */}
      <motion.nav 
        className="hidden md:flex bg-white shadow-xl border-b-4 border-emerald-200 px-8 py-4"
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        ...
        <div className="flex items-center justify-between max-w-7xl mx-auto w-full">
          ...
          >
            ...
            >
              <img 
                src="/image copy.png"
                alt="3Drzewa Logo" 
                className="w-12 h-12 object-cover rounded-full border-2 border-accent-300 shadow-lg"
              />
              <span>3Drzewa</span>
            </Link>
          </motion.div>
            ...
            {navItems.map((item, index) => {
              ...
              return (
                ...
                >
                  <Link
                    to={item.path}
                    className={`flex flex-col items-center py-4 px-2 rounded-2xl transition-all duration-300 min-w-0 flex-1 relative ${
                      isActive
                        ? `text-white`
          ...
          {user && (
            ...
            >
              <Link
                to="/profile"
                className="flex items-center space-x-3 px-6 py-4 rounded-2xl text-sm font-bold text-gray-700 hover:text-accent-700 hover:bg-gray-50 transition-all duration-300"
              >
                ...
                {user.avatar ? (
                  ...
                  <motion.div 
                    className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center border-2 border-gray-300 shadow-lg"
                    whileHover={{ rotate: 360 }}
                    transition={{ duration: 0.5 }}
                  >
      ...
      <div className="md:hidden bg-white shadow-xl border-b-4 border-primary-200 px-4 py-3 flex items-center justify-between">
        <Link 
          to="/" 
          className="flex items-center space-x-3 text-accent-700 font-bold text-xl hover:text-accent-800 transition-colors"
        >
        ...
        {user && (
          <Link
            to="/profile"
            className="flex items-center space-x-2 bg-white hover:bg-gray-50 px-4 py-3 rounded-2xl transition-all duration-300 shadow-lg border border-gray-200"
          >
            <span className="text-sm font-bold text-gray-700">{user.name.split(' ')[0]}</span>
            {user.avatar ? (
              <img src={user.avatar} alt={user.name} className="w-8 h-8 rounded-full border-2 border-emerald-400" />
            ) : (
